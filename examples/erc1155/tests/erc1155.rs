...
#[e2e::test]
async fn constructor_initializes_contract_in_unpaused_state(
    alice: Account,
) -> eyre::Result<()> {
...
#[e2e::test]
async fn balance_of_batch_reverts_when_ids_and_accounts_length_mismatch(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn mint_succeeds_and_updates_balance(alice: Account) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_succeeds_with_receiver_contract(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_reverts_when_receiver_reverts_with_reason(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_reverts_when_receiver_reverts_without_reason(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_reverts_when_receiver_panics(alice: Account) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_batch_succeeds_and_updates_balances(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_batch_succeeds_with_receiver_contract(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_batch_reverts_when_receiver_reverts_with_reason(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_batch_reverts_when_receiver_invalid(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn mint_batch_reverts_when_ids_and_values_length_mismatch(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn set_approval_for_all_updates_approval_status(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn safe_transfer_from_reverts_when_approval_missing(
    alice: Account,
    bob: Account,
    dave: Account,
) -> eyre::Result<()> {
...
#[e2e::test]
async fn safe_batch_transfer_from_reverts_when_receiver_panics(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn safe_batch_transfer_from_reverts_when_receiver_is_zero_address(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn safe_batch_transfer_from_reverts_when_array_length_invalid(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn safe_batch_transfer_from_reverts_when_approval_missing(
    alice: Account,
    bob: Account,
    dave: Account,
) -> eyre::Result<()> {
...
#[e2e::test]
async fn burn_succeeds_and_updates_balance(alice: Account) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn burn_succeeds_with_operator_approval(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn burn_reverts_when_approval_missing(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn burn_reverts_when_balance_insufficient(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn burn_batch_succeeds_and_updates_balances(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn burn_batch_succeeds_with_operator_approval(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn burn_batch_reverts_when_approval_missing(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn burn_batch_reverts_when_balance_insufficient(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn pause_succeeds_and_changes_contract_state(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn unpause_succeeds_and_changes_contract_state(
    alice: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn mint_batch_reverts_when_paused(alice: Account) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn burn_batch_reverts_when_paused(alice: Account) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
    let contract = Erc1155::new(contract_addr, &alice.wallet);
...
#[e2e::test]
async fn safe_batch_transfer_from_reverts_when_paused(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
...
#[e2e::test]
async fn set_approval_for_all_success_when_paused(
    alice: Account,
    bob: Account,
) -> eyre::Result<()> {
    let contract_addr = alice.as_deployer().deploy().await?.address()?;
