...
use alloy::{hex, rpc::types::TransactionReceipt, sol_types::SolConstructor};
...
async fn deploy(account: &Account) -> eyre::Result<TransactionReceipt> {
...
async fn constructs(alice: Account) -> Result<()> {
    ...
    let receipt = deploy(&alice).await?;
...
async fn other_roles_admin_is_the_default_admin_role(
    ...
    let contract_addr = deploy(&alice).await?.address()?;
