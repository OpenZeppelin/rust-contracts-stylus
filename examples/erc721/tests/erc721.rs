...
async fn errors_when_reusing_token_id(alice: User) -> eyre::Result<()> {
    ...
    assert!(err.is_err(Erc721::ERC721InvalidSender { sender: Address::ZERO }));
    Ok(())
}
...
async fn errors_when_transfer_nonexistent_token(
    ...
    assert!(err.is_err(Erc721::ERC721NonexistentToken { tokenId: token_id }));
    Ok(())
}
...
async fn errors_when_transfer_unapproved_token(
    ...
    assert!(err.is_err(Erc721::ERC721InsufficientApproval {
        operator: bob_addr,
        tokenId: token_id,
    }));
    Ok(())
