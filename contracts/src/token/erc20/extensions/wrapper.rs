...
pub trait IERC20Wrapper {
    ...
    ///  Allow a user to deposit underlying tokens and mint the corresponding number of wrapped token
    ///
    /// # Examples
    ///
    /// ```rust,ignore
    ...
    /// }
    /// ```
    fn deposit_to(&self,account:Address, value:U256, erc20: &mut Erc20,  safe_erc20: &mut SafeErc20) -> Result<bool, Self::Error>;
    ...
    /// }
    /// ```
    fn withdraw_to(&self,account:Address, value:U256, erc20: &mut Erc20,  safe_erc20: &mut SafeErc20) -> Result<bool, Self::Error>;
}
...
impl IERC20Wrapper for Erc20Wrapper {
    ...
    fn deposit_to(&self,account:Address, value:U256, erc20: &mut Erc20, safe_erc20: &mut SafeErc20) -> Result<bool, Error> {
        ...
        if sender == contract::address() {
            return  Err(Error::InvalidUnderlying(ERC20InvalidUnderlying { token: account }));
        }
        safe_erc20.safe_transfer_from(underlined_token, sender, contract::address(), value)?;
        erc20._mint(account, value)?;
        Ok(true)
    }

    fn withdraw_to(&self,account:Address, value:U256, erc20: &mut Erc20, safe_erc20: &mut SafeErc20) -> Result<bool, Error> {
        let underlined_token = self._underlying.get();
        if account  == contract::address() {
            return  Err(Error::InvalidUnderlying(ERC20InvalidUnderlying { token: contract::address() }));
        }
