...
mod tests {
    ...
    #[motsu::test]
    fn mints(contract: Contract<Erc721Example>, alice: Address) {
    ...
    #[motsu::test]
    fn safe_mints(contract: Contract<Erc721Example>, alice: Address) {
        let initial_balance = contract
            .sender(alice)
            .balance_of(alice)
            .expect("should return the balance of Alice");
    ...
    fn error_when_safe_mint_invalid_receiver(
        ...
        let err = contract
            .sender(alice)
            .safe_mint(invalid_receiver, TOKEN_ID, vec![0, 1, 2, 3].into())
            .expect_err("should not mint a token for invalid receiver");
    ...
    fn get_approved_nonexistent_token(
        ...
        let approved = contract.sender(alice).erc721._get_approved(TOKEN_ID);
    ...
    fn is_authorized_nonexistent_token(
        ...
        let authorized =
            contract.sender(alice).erc721._is_authorized(alice, bob, TOKEN_ID);
        assert!(!authorized);
    }

    #[motsu::test]
    fn is_authorized_token_owner(
        contract: Contract<Erc721Example>,
        alice: Address,
    ) {
        contract
        ...
        let authorized = contract
            .sender(alice)
            .erc721
            ._is_authorized(alice, alice, TOKEN_ID);
        assert!(authorized);
    }
    ...
    fn check_authorized_token_owner(
        ...
        let result = contract
            .sender(alice)
            .erc721
            ._check_authorized(alice, alice, TOKEN_ID);
    ...
    fn burns(contract: Contract<Erc721Example>, alice: Address) {
        ...
        let result = contract.sender(alice).erc721._burn(TOKEN_ID);
        let balance = contract
            .sender(alice)
            .balance_of(alice)
            .expect("should return the balance of Alice");
    ...
    #[motsu::test]
    fn on_erc721_received(
        erc721: Contract<Erc721Example>,
        receiver: Contract<Erc721ReceiverMock>,
        alice: Address,
    ) {
        erc721
            .sender(alice)
            .safe_mint(receiver.address(), TOKEN_ID, vec![0, 1, 2, 3].into())
            .unwrap();
