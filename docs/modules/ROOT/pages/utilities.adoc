...
- https://docs.rs/openzeppelin-stylus/0.3.0/openzeppelin_stylus/utils/structs/enumerable_set/index.html[`EnumerableSets`]: Contract for managing sets of many primitive types. For information on implementing with custom storage types, see: xref:enumerable-set-custom.adoc[EnumerableSet Implementation for Custom Storage Types].