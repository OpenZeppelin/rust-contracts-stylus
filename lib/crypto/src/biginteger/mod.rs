...
use crate::bits::{bits_to_bytes, BitIterator};
...
pub trait BigInteger:
    ...
    + Zeroize
    + BitIterator
// TODO#q: If we want to implement this trait we don't need to have BITS.
//  Then minimal size will be u64 and conversion should be safe. Possibly remove.
    // + From<u64>
    // + From<u32>
    // + From<u16>
    // + From<u8>
// TODO#q: think about using num traits for it
    + TryFrom<u64>
    + TryFrom<u32>
    + TryFrom<u16>
    + TryFrom<u8>
    // + TryFrom<BigUint, Error = ()> TODO#q: add bigUint conversion
    // + Into<BigUint>
    + FromStr
    + BitXorAssign<Self>
    // + for<'a> BitXorAssign<&'a Self> TODO#q: think about having bit operations on references
...
impl<const BITS: usize, const LIMBS: usize> BigInteger for Uint<BITS, LIMBS> {
    const NUM_LIMBS: usize = 0;

    // TODO#q: do we need to accept `other` by reference?
    fn add_with_carry(&mut self, other: &Self) -> bool {
    ...
    fn mul2(&mut self) -> bool {
        let (res, overflow) = self.overflowing_shl(1);
        *self = res;
        overflow
    }
    ...
    // TODO#q: we don't need `mul*` functions at the moment
    ...
    fn from_bits_be(bits: &[bool]) -> Self {
        let vec = bits_to_bytes(bits).collect::<Vec<_>>();
    ...
    fn find_wnaf(&self, w: usize) -> std::option::Option<std::vec::Vec<i64>> {
        ...
        if (2..64).contains(&w) {
            ...
            while !e.is_zero() {
                let z: i64;
                if e.is_odd() {
                    z = signed_mod_reduction(e.as_limbs()[0], 1 << w);
                    if z >= 0 {
