...
impl<const N: usize> BigInt<N> {
    ...
    pub const fn montgomery_r(&self) -> Self {
        let two_pow_n_times_64 =
            crate::const_helpers::RBuffer::<N>([0u64; N], 1);
...
impl<const N: usize> BigInteger for BigInt<N> {
    ...
    fn add_with_carry(&mut self, other: &Self) -> bool {
        ...
        for i in 0..N {
            carry = arithmetic::adc_for_add_with_carry(
                &mut self.0[i],
                other.0[i],
                carry,
            );
    ...
    fn sub_with_borrow(&mut self, other: &Self) -> bool {
        ...
        for i in 0..N {
            borrow = arithmetic::sbb_for_sub_with_borrow(
                &mut self.0[i],
                other.0[i],
                borrow,
            );
...
impl<const N: usize> TryFrom<BigUint> for BigInt<N> {
    ...
    fn try_from(val: num_bigint::BigUint) -> Result<BigInt<N>, Self::Error> {
        ...
        if bytes.len() > N * 8 {
            ...
            bytes.chunks(8).into_iter().enumerate().for_each(|(i, chunk)| {
