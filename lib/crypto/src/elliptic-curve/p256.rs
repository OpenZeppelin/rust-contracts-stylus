...
impl PrimeCurve for P256 {
    ...
    fn is_member(elem: U256) -> bool {
        FieldElement::ZERO.0 < elem && elem < Self::SUBGROUP_ORDER
    }
}

impl P256 {
    /// Compute the public key from a secret value.
    ///
    /// This is `d * G` where `d` is the secret value, `G` is the curve's base
    /// point, and `*` is scalar multiplication.
    ///
