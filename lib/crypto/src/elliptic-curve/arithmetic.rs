...
use super::{
    affine::AffinePoint, curve::PrimeCurveParams, projective::ProjectivePoint,
};
...
impl<C: PrimeCurveParams> PointArithmetic<C> for C {
    ...
    fn add(
        ...
        let b3 = U256::from(3) * C::EQUATION_B;
    ...
    fn add_mixed(
        ...
        let b3 = C::EQUATION_B * U256::from(3);
        ...
        let mut ret = ProjectivePoint::new(x3, y3, z3);
...
impl<C: PrimeCurveParams> PointArithmetic<C> for EquationAIsMinusThree {
    ...
    fn add(
        ...
        let x3 = (yy_p_bzz3 * xy_pairs) - (yz_pairs * bxz3_part); // 35, 39, 40
        let y3 = (yy_p_bzz3 * yy_m_bzz3) + (xx3_m_zz3 * bxz3_part); // 36, 37, 38
        let z3 = (yy_m_bzz3 * yz_pairs) + (xy_pairs * xx3_m_zz3); // 41, 42, 43
        ProjectivePoint::new(x3, y3, z3)
    }
    ...
    fn add_mixed(
        ...
        let x3 = (yy_p_bzz3 * xy_pairs) - (yz_pairs * bxz3_part); // 28, 32, 33
        let y3 = (yy_p_bzz3 * yy_m_bzz3) + (xx3_m_zz3 * bxz3_part); // 29, 30, 31
        let z3 = (yy_m_bzz3 * yz_pairs) + (xy_pairs * xx3_m_zz3); // 34, 35, 36
        let mut ret = ProjectivePoint::new(x3, y3, z3);
        ret.conditional_assign(lhs, rhs.is_identity());
        ret
    }
