...
pub trait FftField: Field {
    ...
    fn get_root_of_unity(n: u64) -> Option<Self> {
        let mut omega: Self;
        if let Some(large_subgroup_root_of_unity) =
            Self::LARGE_SUBGROUP_ROOT_OF_UNITY
        {
            ...
            let q_adicity = utils::k_adicity(q, n);
            let q_part = q.checked_pow(q_adicity)?;

            let two_adicity = utils::k_adicity(2, n);
            ...
            let log_size_of_group =
                ark_std::log2(usize::try_from(size).expect("too large"));
