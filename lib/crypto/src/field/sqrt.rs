...
pub enum SqrtPrecomputation<F: Field> {
    // Tonelli-Shanks algorithm works for all elements, no matter what the
    // modulus is.
    TonelliShanks {
        two_adicity: u32,
    ...
    Case3Mod4 { modulus_plus_one_div_four: &'static [u64] },
}
    ...
    pub fn sqrt(&self, elem: &F) -> Option<F> {
        match self {
            Self::TonelliShanks {
                ...
                // Actually this is just normal Tonelli-Shanks; since
                // `P::Generator` is a quadratic non-residue,
                // `P::ROOT_OF_UNITY = P::GENERATOR ^ t` is also
                // a quadratic non-residue (since `t` is odd).
                if elem.is_zero() {
                    return Some(F::zero());
                }
                ...
                while !b.is_one() {
                    ...
                    if k == (*two_adicity as usize) {
                        // We are in the case where self^(T * 2^k) =
                        // x^(P::MODULUS - 1) = 1, which
                        // means that no square root exists.
                        return None;
                    }
                    let j = v - k;
                    w = z;
            ...
            Self::Case3Mod4 { modulus_plus_one_div_four } => {
