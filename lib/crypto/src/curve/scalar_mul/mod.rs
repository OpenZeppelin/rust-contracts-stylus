    ...
    + From<Self::MulBase>
{
    ...
    /// - a list `v` of n scalars
    ///
    /// This method allows the user to provide a precomputed table of multiples
    /// of `g`. A more ergonomic way to call this would be to use
    /// [`BatchMulPreprocessing::batch_mul`].
    ///
    /// # Example
    /// ```
    /// use ark_std::{One, UniformRand};
...
/// Preprocessing used internally for batch scalar multiplication via
/// [`ScalarMul::batch_mul`].
/// - `window` is the window size used for the precomputation
/// - `max_scalar_size` is the maximum size of the scalars that will be
///   multiplied
...
impl<T: ScalarMul> BatchMulPreprocessing<T> {
    ...
    pub fn with_num_scalars_and_scalar_size(
        ...
        let table = cfg_iter!(multiples_of_g)
            ...
            .collect();
        Self { window, max_scalar_size, table }
    }
    ...
    pub fn batch_mul(&self, v: &[T::ScalarField]) -> Vec<T::MulBase> {
        let result: Vec<_> =
            cfg_iter!(v).map(|e| self.windowed_mul(e)).collect();
    ...
    fn windowed_mul(&self, scalar: &T::ScalarField) -> T {
        ...
        for outer in 0..outerc {
            ...
            for i in 0..self.window {
                if outer * self.window + i < modulus_size
                    && scalar_val[outer * self.window + i]
                {
                    inner |= 1 << i;
                }
            }
