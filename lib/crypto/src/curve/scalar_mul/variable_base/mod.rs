...
pub trait VariableBaseMSM: ScalarMul {
    /// Computes an inner product between the [`PrimeField`] elements in
    /// `scalars` and the corresponding group elements in `bases`.
    ///
        ...
        J::Item: Borrow<Self::MulBase>,
    {
        ...
        // align the streams
        // TODO: change `skip` to `advance_by` once rust-lang/rust#7774 is
        // fixed. See <https://github.com/rust-lang/rust/issues/77404>
        let mut bases =
            bases_init.skip(bases_stream.len() - scalars_stream.len());
        let step: usize = 1 << 20;
...
fn msm_bigint_wnaf<V: VariableBaseMSM>(
    ...
    let c = if size < 32 { 3 } else { super::ln_without_floats(size) + 2 };
...
fn msm_bigint<V: VariableBaseMSM>(
    ...
    let window_sums: Vec<_> = ark_std::cfg_into_iter!(window_starts)
        .map(|w_start| {
            ...
            // Compute sum_{i in 0..num_buckets} (sum_{j in i..num_buckets}
            // bucket[j]) This is computed below for b buckets,
            // using 2b curve additions.
            //
...
fn make_digits(
    ...
    let mut carry = 0u64;
    let num_bits = if num_bits == 0 { a.num_bits() as usize } else { num_bits };
