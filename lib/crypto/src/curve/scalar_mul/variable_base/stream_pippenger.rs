...
pub struct HashMapPippenger<G: VariableBaseMSM> {
    buffer: HashMap<
        G::MulBase,
        G::ScalarField,
        core::hash::BuildHasherDefault<DefaultHasher>,
    >,
    result: G,
    buf_size: usize,
}
...
impl<G: VariableBaseMSM> HashMapPippenger<G> {
    ...
    {
        // update the entry, guarding the possibility that it has been already
        // set.
        let entry =
            self.buffer.entry(*base.borrow()).or_insert(G::ScalarField::zero());
        *entry += *scalar.borrow();
