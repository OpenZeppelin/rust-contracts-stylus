...
pub trait SWCurveConfig: super::CurveConfig {
    ...
    /// If uncompressed, serializes both x and y coordinates as well as a bit
    /// for whether it is infinity. If compressed, serializes x coordinate
    /// with two bits to encode whether y is positive, negative, or
    /// infinity.
    #[inline]
    ...
    /// If `validate` is `Yes`, calls `check()` to make sure the element is
    /// valid.
        ...
        let (x, y, flags) = match compress {
            Compress::Yes => {
                let (x, flags): (_, SWFlags) =
                    CanonicalDeserializeWithFlags::deserialize_with_flags(
                        reader,
                    )?;
                match flags {
                    ...
                    _ => {
                        ...
                        let (y, neg_y) =
                            Affine::<Self>::get_ys_from_x_unchecked(x)
    ...
    fn serialized_size(compress: Compress) -> usize {
        ...
        match compress {
            ...
            Compress::No => {
                zero.compressed_size()
                    + zero.serialized_size_with_flags::<SWFlags>()
            }
        }
    }
}
